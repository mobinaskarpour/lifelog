...
class TimelineRepositoryImpl @Inject constructor(
    ...
    override fun getEventsBetween(start: Long, end: Long): Flow<List<TimelineEvent>> =
        dao.getBetween(start, end).map { list -> list.map { it.toDomain() } }

    override fun searchEvents(keyword: String): Flow<List<TimelineEvent>> =
        dao.search(keyword).map { list -> list.map { it.toDomain() } }

    override suspend fun insertEvent(event: TimelineEvent): Long = withContext(dispatchers.io) {
        dao.insert(event.toEntity())
    }

    override suspend fun insertEvents(events: List<TimelineEvent>) = withContext(dispatchers.io) {
        dao.insertAll(events.map { it.toEntity() })
    }

    override suspend fun deleteOldEvents(beforeTimestamp: Long) = withContext(dispatchers.io) {
...
class AppUsageRepositoryImpl @Inject constructor(
    ...
    override fun getUsageForDate(date: String): Flow<List<AppUsage>> =
        dao.getForDate(date).map { list -> list.map { it.toDomain() } }

    override fun getUsageBetween(startDate: String, endDate: String): Flow<List<AppUsage>> =
        dao.getBetween(startDate, endDate).map { list -> list.map { it.toDomain() } }

    override fun getTopApps(limit: Int): Flow<List<AppUsage>> =
        dao.getTop(limit).map { list -> list.map { it.toDomain() } }

    override suspend fun insertOrUpdateUsage(usage: AppUsage) = withContext(dispatchers.io) {
        dao.insert(usage.toEntity())
    }

    override suspend fun deleteOldUsage(beforeDate: String) = withContext(dispatchers.io) {
...
class NotificationRepositoryImpl @Inject constructor(
    ...
    override suspend fun deleteOldNotifications(beforeTimestamp: Long) =
        withContext(dispatchers.io) { dao.deleteBefore(beforeTimestamp) }
}
...
class CallRepositoryImpl @Inject constructor(
    ...
    override fun searchCalls(keyword: String): Flow<List<CallLog>> =
        dao.search(keyword).map { list -> list.map { it.toDomain() } }
    ...
    override suspend fun insertCall(call: CallLog): Long =
        withContext(dispatchers.io) { dao.insert(call.toEntity()) }

    override suspend fun deleteOldCalls(beforeTimestamp: Long) =
        withContext(dispatchers.io) { dao.deleteBefore(beforeTimestamp) }
}
...
class LocationRepositoryImpl @Inject constructor(
    ...
    override fun getLocationsBetween(start: Long, end: Long): Flow<List<LocationLog>> =
        dao.getBetween(start, end).map { list -> list.map { it.toDomain() } }

    override suspend fun insertLocation(location: LocationLog): Long =
        withContext(dispatchers.io) { dao.insert(location.toEntity()) }

    override suspend fun deleteOldLocations(beforeTimestamp: Long) =
        withContext(dispatchers.io) { dao.deleteBefore(beforeTimestamp) }
}
...
class BatteryRepositoryImpl @Inject constructor(
    ...
    override fun getLatestBatteryLog(): Flow<BatteryLog?> =
        dao.getLatest().map { it?.toDomain() }

    override suspend fun insertBatteryLog(log: BatteryLog): Long =
        withContext(dispatchers.io) { dao.insert(log.toEntity()) }

    override suspend fun deleteOldBatteryLogs(beforeTimestamp: Long) =
        withContext(dispatchers.io) { dao.deleteBefore(beforeTimestamp) }
}

@Singleton
class ScreenEventRepositoryImpl @Inject constructor(
    ...
    override suspend fun insertScreenEvent(event: ScreenEvent): Long =
...
class SettingsRepositoryImpl @Inject constructor(
    ...
    override suspend fun updateTheme(themeMode: ThemeMode) = withContext(dispatchers.io) {
        val current = dataStore.settings.first()
        dataStore.updateSettings(current.copy(themeMode = themeMode))
    }

    override suspend fun updateLanguage(language: Language) = withContext(dispatchers.io) {
        val current = dataStore.settings.first()
...
class ExportRepositoryImpl @Inject constructor(
    ...
    override suspend fun exportToPdf(outputPath: String): Boolean = withContext(dispatchers.io) {
        try {
    ...
    override suspend fun backupDatabase(outputPath: String): Boolean = withContext(dispatchers.io) {
        try {
    ...
    override suspend fun restoreDatabase(inputPath: String): Boolean = withContext(dispatchers.io) {
        try {
